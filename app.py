...
# --- Migraciones suaves (BD antiguas) ---
try:
    c.execute("ALTER TABLE picking_tasks ADD COLUMN family TEXT;")
except Exception:
    pass
try:
    c.execute("ALTER TABLE picking_ots ADD COLUMN model TEXT;")
except Exception:
    pass
...
def load_master_from_path(path: str) -> tuple[dict, dict, dict, list]:
    """Carga maestro SKU/EAN desde Excel.

    Retorna:
      - inv_map_sku: {sku -> titulo_tecnico}
      - familia_map_sku: {sku -> Familia}
      - barcode_to_sku: {ean/barcode -> sku}
      - conflicts: lista de conflictos (barcode, sku_a, sku_b)
    """
...
def save_orders_and_build_ots(
    ...
    model:
      - "VENTAS": (actual) reparte ventas a OTs y crea tareas por SKU dentro de esas ventas.
      - "SKU": agrupa SKUs de todas las ventas y asigna por Familia a los pickeadores (batch picking).
    """
    ...
    # Reset corrida (sin borrar orders/order_items: se vuelven a recrear/actualizar por ml_order_id)
    ...
    # 1) Upsert orders + items
    order_id_by_ml: dict[str, int] = {}
    ...
    for i in range(int(num_pickers)):
        ...
        picker_ids.append(int(c.lastrowid))

    # 3) Crear OTs (guardando modelo)
    ot_ids: list[int] = []
        ...
        for ot_id in ot_ids:
            ...
            for sku, title, title_tec_any, total in rows:
                ...
                if sku in cortes_set:
                    c.execute(
                        """INSERT INTO cortes_tasks (ot_id, sku_ml, title_ml, title_tec, qty_total, created_at)
                           VALUES (?,?,?,?,?,?)""",
    ...
    # título ML por SKU (primero no vacío)
    title_ml_by_sku: dict[str, str] = {}
    ...
    for ot_id in ot_ids:
        ...
        # orden: familia, sku numérico
        try:
            sub["_sku_num"] = pd.to_numeric(sub["sku_ml"], errors="coerce")
        except Exception:
            sub["_sku_num"] = None
        sub = sub.sort_values(["family", "_sku_num", "sku_ml"], ascending=[True, True, True])

        for _, r in sub.iterrows():
            sku = normalize_sku(r["sku_ml"])
            qty_total = int(r["qty"] or 0)
            if not sku or qty_total <= 0:
                continue

            family = str(r["family"] or "").strip() or "Sin Familia"

            title_tec = inv_map_sku.get(sku, "")
            title_ml = title_ml_by_sku.get(sku, "")
            title_eff = title_tec if title_tec else title_ml
            ...
            if sku in cortes_set:
                c.execute(
                    """INSERT INTO cortes_tasks (ot_id, sku_ml, title_ml, title_tec, qty_total, created_at)
                       VALUES (?,?,?,?,?,?)""",
                    (ot_id, sku, title_eff, title_tec, qty_total, now_iso())
                )
            else:
                c.execute(
                    """INSERT INTO picking_tasks (ot_id, sku_ml, title_ml, title_tec, qty_total, qty_picked, status, decided_at, confirm_mode, family)
                       VALUES (?,?,?,?,?,?,?,?,?,?)""",
                    (ot_id, sku, title_eff, title_tec, qty_total, 0, "PENDING", None, None, family)
                )
...
def page_import(inv_map_sku: dict, familia_map_sku: dict):
    ...
    model_label = st.radio("Modelo de picking", ["Por ventas", "Por SKU"], horizontal=True)
    model = "VENTAS" if model_label == "Por ventas" else "SKU"

...
def page_app_lobby():
    ...
    st.markdown('<div class="lobbybtn">', unsafe_allow_html=True)
    if st.button("📦 Embalador (mesas)", key="mode_packer"):
        st.session_state.app_mode = "PACKER"
        st.rerun()
    st.markdown("</div>", unsafe_allow_html=True)
    st.caption("Embalaje rápido por mesa (post-sorting).")
...
def _s2_create_tables():
    ...
    # Embalador: registro de ventas embaladas (por mesa)
...
# =========================
# UI: EMBALADOR (post-sorting v2)
# =========================
def _s2_get_packed_map(mid: int, mesa: int) -> dict:
    """Retorna {sale_id: packed_at} para la mesa indicada."""
    _s2_create_tables()
    conn = get_conn()
    c = conn.cursor()
    c.execute(
        "SELECT sale_id, packed_at FROM s2_packing WHERE manifest_id=? AND mesa=?;",
        (mid, int(mesa)),
    )
    rows = c.fetchall()
    conn.close()
    return {str(sid): pat for sid, pat in rows}

def _s2_pack_sale(mid: int, mesa: int, sale_id: str, operator: str, shipment_id: str|None, pack_id: str|None):
    _s2_create_tables()
    conn = get_conn()
    c = conn.cursor()
    c.execute(
        """INSERT INTO s2_packing (manifest_id, sale_id, mesa, operator, packed_at, status, shipment_id, pack_id)
           VALUES (?,?,?,?,?,?,?,?)
           ON CONFLICT(manifest_id, sale_id) DO UPDATE SET
             mesa=excluded.mesa,
             ...
             pack_id=excluded.pack_id;
        """,
        (mid, str(sale_id), int(mesa), str(operator or "").strip(), _s2_now_iso(), "PACKED",
         str(shipment_id or "").strip() or None, str(pack_id or "").strip() or None)
    )
    conn.commit()
    conn.close()

def _s2_find_done_sale_in_mesa(mid: int, mesa: int, token: str) -> dict:
    """Busca venta DONE en una mesa por shipment_id o pack_id (token numérico)."""
    ...
    # 1) match directo por shipment_id o pack_id en s2_sales
    c.execute(
        """SELECT sale_id, shipment_id, pack_id, customer, page_no, closed_at
           FROM s2_sales
           WHERE manifest_id=? AND mesa=? AND status='DONE'
             AND (shipment_id=? OR pack_id=?)
           LIMIT 1;
        """,
        (mid, int(mesa), tok, tok)
    )
    row = c.fetchone()

    # 2) si no, intentar map pack->shipment (Colecta) y buscar por shipment_id
    if not row:
        c.execute("SELECT shipment_id FROM s2_pack_ship WHERE manifest_id=? AND pack_id=? LIMIT 1;", (mid, tok))
        r2 = c.fetchone()
        if r2 and r2[0]:
            ship = str(r2[0]).strip()
            c.execute(
    ...
    return {
        "sale_id": str(row[0]),
        "shipment_id": str(row[1] or "").strip(),
        "pack_id": str(row[2] or "").strip(),
        "customer": str(row[3] or "").strip(),
        "page_no": int(row[4]) if row[4] is not None else None,
        "closed_at": str(row[5] or "").strip(),
    }

def _s2_get_sale_items(mid: int, sale_id: str) -> list[dict]:
    _s2_create_tables()
    conn = get_conn()
    c = conn.cursor()
    c.execute(
        """SELECT sku, description, qty
           FROM s2_items
    ...
    for sku, desc, qty in rows:
        out.append({"sku": str(sku), "desc": str(desc or ""), "qty": int(qty or 0)})
    return out

def _s2_label_raw(mid: int, shipment_id: str) -> str:
    _s2_create_tables()
    conn = get_conn()
    c = conn.cursor()
    c.execute("SELECT raw FROM s2_labels WHERE manifest_id=? AND shipment_id=?;", (mid, str(shipment_id)))
    row = c.fetchone()
    conn.close()
    return str(row[0] or "") if row else ""

def _s2_guess_recipient_address_from_label(raw: str) -> tuple[str, str]:
    """Best-effort: intenta extraer (destinatario, dirección) desde ZPL/TXT."""
    s = str(raw or "")
    if not s:
        return "", ""
    # Tomar valores de ^FD ... ^FS y filtrar cosas que no sirven
    vals = re.findall(r"\^FD\s*([^^]{2,120}?)\s*\^FS", s)
    cleaned = []
    bad_words = ["pack", "venta", "mercado", "libre", "carrier", "codigo", "código", "ruta", "env", "shipment", "id:"]
    for v in vals:
        ...
        if any(b in low for b in bad_words):
            continue
        # descartar si es casi puro número
        if re.fullmatch(r"[0-9 \-]{6,}", vv):
            continue
        cleaned.append(vv)

    # Heurística: primer texto "humano" -> destinatario, siguiente -> dirección
    if not cleaned:
        return "", ""
    destinatario = cleaned[0]
    direccion = cleaned[1] if len(cleaned) > 1 else ""
    return destinatario, direccion

def page_packer(inv_map_sku: dict):
    _s2_create_tables()
    st.title("Embalador")
    st.caption("Embalaje rápido por mesa (post-sorting). Solo se habilita cuando el camarero cierra la venta (status DONE).")

    mid = _s2_get_active_manifest_id()
    st.session_state["packing_manifest_id"] = mid

    # Operador (simple)
    operator = st.text_input("Operador embalaje", value=str(st.session_state.get("packing_operator", "E1")))
    st.session_state["packing_operator"] = operator.strip() or "E1"

    # Mesas disponibles (con ventas DONE)
    conn = get_conn()
    c = conn.cursor()
    c.execute(
        """SELECT mesa, COUNT(1) 
           FROM s2_sales
           WHERE manifest_id=? AND mesa IS NOT NULL AND status='DONE'
           GROUP BY mesa
           ORDER BY mesa;""",
        (mid,)
    )
    mesa_rows = c.fetchall()
    conn.close()

    if not mesa_rows:
        st.info("Aún no hay mesas listas para embalar. Primero: Cargar control/etiquetas, asignar mesas y cerrar ventas desde Camarero.")
        return

    mesas = [int(r[0]) for r in mesa_rows]
    mesa = st.selectbox("Mesa", options=mesas, index=0, key="packing_mesa_sel")

    # Resumen mesa
    packed_map = _s2_get_packed_map(mid, int(mesa))
    conn = get_conn()
    c = conn.cursor()
    c.execute(
        """SELECT COUNT(1) 
    ...
    total_done = int(c.fetchone()[0] or 0)
    conn.close()
    packed_count = len(packed_map)
    pending_count = max(0, total_done - packed_count)

    a, b, d = st.columns(3)
    a.metric("Ventas en mesa (listas)", total_done)
    b.metric("Embaladas", packed_count)
    d.metric("Pendientes", pending_count)
    ...
    df = pd.DataFrame(
        rows,
        columns=["Venta", "Shipment", "Pack", "Cliente", "Página", "Cerrada"],
    )
    df["Estado"] = df["Venta"].map(lambda sid: "✅ EMBALADA" if str(sid) in packed_map else "🟦 PENDIENTE")
    df["Embalada a las"] = df["Venta"].map(lambda sid: to_chile_display(packed_map.get(str(sid), "")) if str(sid) in packed_map else "")
    st.subheader("Mesa — Lista de ventas")
    st.dataframe(df[["Estado", "Venta", "Cliente", "Shipment", "Pack", "Página", "Cerrada", "Embalada a las"]], use_container_width=True, hide_index=True)

    st.divider()

    # Escaneo etiqueta
    st.subheader("Escanear etiqueta")
    if st.session_state.get("_packing_clear_scan"):
        st.session_state["packing_scan_widget"] = ""
        st.session_state["_packing_clear_scan"] = False

    scan = st.text_input("Etiqueta (QR/barra)", key="packing_scan_widget")
    if not scan:
        st.info("Escanea una etiqueta para abrir el pedido en pantalla grande.")
        return

    token = _s2_extract_shipment_id(scan)
    if not token:
        st.error("No pude leer un ID desde el escaneo.")
        sfx_emit("ERR")
        return

    sale = _s2_find_done_sale_in_mesa(mid, int(mesa), token)
    if not sale:
        st.error("Esa etiqueta no corresponde a una venta cerrada (DONE) de esta mesa.")
        sfx_emit("ERR")
        return

    sale_id = sale["sale_id"]
    if sale_id in packed_map:
        st.warning("⚠️ Esta venta ya está marcada como EMBALADA.")
        sfx_emit("ERR")
        return

    # Vista foco: datos etiqueta + productos
    st.markdown("## Pedido activo")
    top1, top2 = st.columns([2, 1])
    with top1:
        st.markdown(f"### Venta: **{sale_id}**  |  Mesa: **{mesa}**  |  Página: **{sale.get('page_no','')}**")
        if sale.get("customer"):
            st.caption(f"Cliente (control): {sale.get('customer')}")
    with top2:
        st.markdown("")

    ship = sale.get("shipment_id") or ""
    pack = sale.get("pack_id") or ""
    st.markdown(f"**N° envío (Shipment):** {ship if ship else '—'}")
    st.markdown(f"**Pack ID:** {pack if pack else '—'}")

    raw = _s2_label_raw(mid, ship) if ship else ""
    destinatario, direccion = _s2_guess_recipient_address_from_label(raw)
    if destinatario or direccion:
        st.markdown(f"**Destinatario:** {destinatario if destinatario else '—'}")
        st.markdown(f"**Dirección:** {direccion if direccion else '—'}")
    else:
        st.info("No se pudo extraer destinatario/dirección automáticamente desde la etiqueta. (Puedes abrir el raw abajo).")

    with st.expander("Ver etiqueta (raw)", expanded=False):
        st.code(raw[:4000] if raw else "(sin raw guardado)")

    st.divider()

    items = _s2_get_sale_items(mid, sale_id)
    if not items:
        st.warning("No encontré productos para esta venta en el Control.")
    else:
        st.subheader("Productos")
        for it in items:
            sku = it["sku"]
            qty = it["qty"]
            title = it["desc"] or inv_map_sku.get(sku, "") or sku

            # foto (si existe)
            urls, link = get_picture_urls_for_sku(sku)
            rowA, rowB = st.columns([1, 3])
            with rowA:
                if urls:
            ...
            with rowB:
                st.markdown(f"**{sku}**")
                st.markdown(f"{title}")
                st.markdown(f"### Cantidad: **{qty}**")

            st.divider()

    # Acción final
    if st.button("✅ Venta embalada", type="primary", use_container_width=True):
        _s2_pack_sale(mid, int(mesa), sale_id, st.session_state.get("packing_operator", "E1"), ship, pack)
        st.session_state["_packing_clear_scan"] = True
        sfx_emit("OK")
        st.success("Marcada como EMBALADA ✅")
        st.rerun()
